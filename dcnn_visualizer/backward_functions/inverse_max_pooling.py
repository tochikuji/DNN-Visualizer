...
def __sparse_max_unpooling_inner(node, pooled, raw, is_positional):
    ...
    for i, (x0, y0, x1, y1) in enumerate(roi_iter):
        ...
        if is_positional:
            ...
            for ch_i, (py, px) in enumerate(max_locations):
                unpooled[ch_i, y0 + py, x0 + px] = pooled[ch_i, pooled_y, pooled_x].data

        else:
            # is not positional
            px, py = __center(x0, y0, x1, y1)
            unpooled[:, y0 + py, x0 + px] = pooled[:, pooled_y, pooled_x].data
...
def __max_location(receptive_field_raw):
    max_locations = [numpy.argwhere(c.data == c.data.max())[0] for c in receptive_field_raw]
